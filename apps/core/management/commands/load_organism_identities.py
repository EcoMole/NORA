...
from novel_food.models import NovelFood, Organism, OrgType, Family, Genus, Species, NovelFoodOrganism, SynonymType, OrganismSyn
...
class Command(BaseCommand):
    ...
    def add_organism_identity(self, row):
        ...
        if not pd.isna(row['vocab']): # We know its in vocabulary
            organism_vocab = TaxonomyNode.objects.get(extended_name=row['vocab'].strip(), taxonomy__code="MTX")
        else:
            final_msg += 'Organism not found in vocabulary, created dummy node -> add it.\n'
            organism_vocab = TaxonomyNode.objects.get(extended_name='Dummy node', taxonomy__code="MTX", code='NORA')
        ...
        if not pd.isna(row['family']):
            family_obj = Family.objects.get_or_create(title=row['family'], org_type=org_type_obj)
            genus_obj = Genus.objects.get_or_create(title=row['genus'], family=family_obj[0])
        ...
        nf_organism = NovelFoodOrganism.objects.create(organism=organism_obj, novel_food=novel_food_obj)
        ...
        if not pd.isna(r_common_names):
            ...
            for common_name in common_names:
                OrganismSyn.objects.create(organism=organism_obj, synonym=common_name, type=common_name_synonym_type)
    ...
    def handle(self, *args, **options):
        ...
        TaxonomyNode.objects.get_or_create(extended_name='Dummy node', taxonomy=Taxonomy.objects.get(code='MTX'), code='NORA')
