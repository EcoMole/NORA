...
class Command(BaseCommand):
    ...
    def create_novel_food_categories(self):
        ...
        article_3 = TaxonomyNode.objects.get_or_create(code='NORA', taxonomy = legref, short_name='Regulation (EC) No 2015/2283 Article 3', 
        ...
        for key, value in categories_2015_2283.items():
            Category.objects.get_or_create(title=key, definition=value, regulation=article_3[0])
        ...
        for key, value in categories_97_618.items():
            Category.objects.get_or_create(title=key, definition=value, regulation=recommendation[0])
    ...
    def create_food_forms(self):
        options = ['powder', 'paste', 'whole frozen', 'whole dried', 'juice', 'pulp', 'extract', 'decorticated grain']
        for option in options:
    ...
    def create_parameter_types(self):
        options = ['Microbiological', 'Mycotoxins', 'Heavy metals', 'Minerals', 'Hoodigosides', 'Vitamins']
        for option in options:
            ParameterType.objects.get_or_create(title=option)

    def create_proposed_use_types(self):
        options = ["whole_foods", "food_ingredients", "food_supplements", "infant_follow_on_formula", "special_medical_purpose", "total_diet_replacement"]
        for option in options:
