...
def duplicate_model(modeladmin, request, queryset):
    def handle_unique_true_fields(obj):
        """
        This method will prepend "DUPLICATE_" to the values of fields which has
        unique=true to avoid unique constraint violations.
        This method is used on newly created duplicate, represented as parameter `obj`.
        """
                    ...
                    if obj.__class__.objects.filter(
                        **{field.name: duplicated_value}
                        ...
                        raise ValidationError(
                            ...
                            "unique=True and there is already an "
                            f"instance with {field.name}={duplicated_value} in the database."
                        )
                    else:
    ...
    def duplicate_related_objects(original_obj, duplicated_obj):
        ...
        """
        if (
            hasattr(original_obj, "duplicate_related")
            and original_obj.duplicate_related
        ):
            if not isinstance(original_obj.duplicate_related, (list, tuple, set)):
                raise ValidationError(
                    "Model attribute duplicate_related has to "
                    "be type of list, tuple, or set."
                )
